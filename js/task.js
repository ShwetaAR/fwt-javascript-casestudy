...
Task.prototype.add=function(taskValue,dueDate,status){
	status=false;
	console.log(dueDate.value)
	listTask.push({taskValue, dueDate,status});
	showTask();
...
Task.prototype.deleteByIndex=function(id){
    listTask.splice(id,1);
     
}

Task.prototype.changeStatusByIndex=function(id){
	 if( listTask[id].status==false){
  	 listTask[id].status = true;
  	}
  	else
  		 listTask[id].status = false;
...
function showTask(){
	task = new Task();
	this.listTask=task.getTask();
    createAndAppendList(listTask);
...
function deleteTask() {
    ...
    createAndAppendList(task.getTask());
...
function filterTasks(){
	...
	this.listTask = task.getTask();
	this.iProgressList;
	for (var index = 0; index < listTask.length; index++) {
		var element = listTask[index];
		if(element.status==false){
			this.iProgressList.push(listTask[index]);
		}
	}
createAndAppendList(this.iProgressList);
	...
	for (var index = 0; index < listTask.length; index++) {
		var element = listTask[index];
		if(element.status==true){
			this.completeList.push(listTask[index]);
		}
	}
...
function checkAll(){
	...
	console.log(allCheckBox.value);
        for (index = 0 ; index<checkboxes.length ; index ++){
        		checkboxes[index].checked = allCheckBox;
         }
}
